...
if(isset($_GET['cat_list']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['category_image'] = $file_path.'imagess/menuImages/'.$data['category_image'];
			if (!strpos($data['category_image'], '_')) {
                $row['category_image'] = $file_path.'imagess/menuImages/Image-Coming-Soon.png';
            }
...
else if(isset($_GET['slider_list']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['banner_name_imagepath'] = $file_path.'imagess/bannerImages/'.$data['banner_image'];
			...
			$reponseobj['response']->message = 'received data';
...
function get_order_unique_id()
{
		...
		for ($i = 0; $i < sizeof($menu); $i++) {
			...
			while($data = mysqli_fetch_assoc($sql))
			{
				...
				$sql = "INSERT INTO tbl_order_details(order_id,cart_id, user_id, menu_id, total_price, payment_id,payment_type,menu_qty,cid) values 
							('".$order_id."','".$cart_id."', '".$user_id."', '".$menu_id."','".$total_price."','".$payment_id."','".$payment_type."','".$menu_qty."','".$cid."')";
...
//payment ios start
else if (isset($_REQUEST['payment_detail_ios'])) {
    function get_order_unique_id()
    {
    ...
    $sql = "INSERT INTO tbl_order_details(order_unique_id, cart_id, user_id, menu_id, total_price, cid) values 
							('" . $order_unique_id . "','" . $cart_id . "', '" . $user_id . "', '" . $menu_id . "','" . $total_price . "', '".$cid."')";
    mysqli_query($mysqli, $sql);
		...
		if($rowcount>0)
 		{
			while($data = mysqli_fetch_assoc($sql))
			{
				...
				$row['menu_image'] = $file_path.'imagess/menuImages/'.$data['menu_image'];
...
{		  
			...
			while($data = mysqli_fetch_assoc($sql1))
		{
			...
			$set['response']->message = 'received data';
			$set['response']->code = 200; 
	...
	function get_menu_image($menu_id)
     {
		...
		return $file_path.'https://'.$_SERVER['SERVER_NAME'] . dirname($_SERVER['REQUEST_URI']).'/imagess/menuImages/'.$data1['menu_image'];
     }
...
else if(isset($_GET['promo_code']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$reponseobj['message'] = 'received data';
			$reponseobj['code'] = 200;    					
...
{      
	...
	{ 
		if($data['status'] == 1)
		{
			if ($data['promo_start_date'] <= $today_date && $data['promo_end_date'] >= $today_date) 
			{
				...
				$querydd = mysqli_query($mysqli,"select SUM(menu_total_price)as total from tbl_order_items where order_id = '".$_GET['order_id']."'");
					...
					{
						$reponseobj['message'] = 'promo applied';
						$reponseobj['code'] = 200; 
					}
					else
					{
						
						$reponseobj['message'] = 'unable to apply promo';
						$reponseobj['code'] = 404;    	
					}
			...
			{
				$reponseobj['message'] = 'promo has expired';
				$reponseobj['code'] = 404;    					
			}
		...
		{
    		$reponseobj['message'] = 'promo is inactive';
			$reponseobj['code'] = 404;    					
		}
	}
	else
	{
			$reponseobj['message'] = 'promo not found';
			$reponseobj['code'] = 404;
	}
...
//api oreder summary start
    ...
    $query = "SELECT * FROM tbl_order_details WHERE user_id = '" . $_GET['order_summary'] . "' ORDER BY id DESC";
    //var_dump($query);
    $sql = mysqli_query($mysqli, $query);
    $rowcount = mysqli_num_rows($sql);
    ...
    if ($rowcount > 0) {
        ...
        $set['message'] = 'Retrived Data From OSO';
        $set['code'] = 200;
        $set['user_order_list'] = $jsonObj;
    } else {
        $set['message'] = 'No Orders Yet';
        $set['code'] = 404;
...
{		  
		...
		$query="SELECT * FROM tbl_menu_list WHERE tbl_menu_list.mid='".$_GET['menu_multiple_image']."'";
				
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
		while($data = mysqli_fetch_assoc($sql))
		{
			$row['mid'] = $data['mid'];
			$row['items_images'] = $file_path.'imagess/menuImages/'.$data['menu_image'];
...
{		  
    ...
    $query = "SELECT * FROM tbl_menu_list
				LEFT JOIN tbl_category ON tbl_category.cid= tbl_menu_list.cid
...
else if(isset($_GET['orderdelivery_detail']))
{
     ...
     function get_menu_image($menu_id)
     {
		...
		return $file_path.'images/menuImages/'.$data1['menu_image'];
     }
...
else if(isset($_GET['menucat_list']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$set['response']->message = 'received data';
			$set['response']->code = 200;    
			$set['response']->urlpath = $file_path.'imagess/menuImages/'.$data['category_image'];
			if (!strpos($data['category_image'], '_')) {
               	$set['response']->urlpath = $file_path.'imagess/extra/Image-Coming-Soon.png';
            }
...
else if(isset($_GET['oso_open']))
{
    ...
    while($data = mysqli_fetch_assoc($sql))
    {
        ...
        $reponseobj['message'] = 'Retrieved Data From OSO';
        $reponseobj['code'] = 200;
        $reponseobj['status'] = $data['status'];
    }
    header( 'Content-Type: application/json; charset=utf-8' );
...
else 
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['oso_name'] = $data['oso_name'];
			$row['oso_logo'] = $data['oso_logo'];
			$row['oso_version'] = $data['oso_version'];
			$row['oso_author'] = $data['oso_author'];
			$row['oso_contact'] = $data['oso_contact'];
			$row['oso_email'] = $data['oso_email'];
			$row['oso_website'] = $data['oso_website'];
			$row['oso_description'] = stripslashes($data['oso_description']);
 			$row['oso_developed_by'] = $data['oso_developed_by'];

			$row['app_privacy_policy'] = stripslashes($data['oso_privacy_policy']); 	
