<?php 
	...
	include('includes/function.php');
		...
		if($_FILES['user_image']['name']!='')
        {
               ...
               //Main Image
               $tpath1='imagess/userImages/'.$user_image;       
               $pic1=compress_image($_FILES["user_image"]["tmp_name"], $tpath1, 80);           
               
               $user_image='imagess/userImages/'.$user_image;
        }
        else
        {
	...
	/*	if($_POST['address_line_1']!="")
		...
		*/
		if($_FILES['user_image']['name']!='')
		{
			mysqli_query($mysqli,"UPDATE tbl_users SET user_image ='imagess/userImages/'.$user_image WHERE id = $id");
		}
			...
			$set['response']->message = 'Profile Updated';
